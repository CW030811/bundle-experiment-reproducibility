...
def verify_table5(result_csv=None):
    registry = json.loads((ROOT / 'provenance/PUBLISHED_VALUES.json').read_text())
    published = registry['table5_rows']
    ...
    for target in published:
        for method, offset in [('FCP', 2), ('BSP', 5), ('CPBSD-A', 8)]:
            src_rows = ca_data if method == 'CPBSD-A' else data
            subset = [r for r in src_rows if r['scale'] == f'N{target[0]}_K50' and r['cost'] == target[1]
    ...
    superseded = []
    for key, cell in registry.get('table5_cpbsd_a_rerun', {}).get('cells', {}).items():
        for field in ('ins', 'oos'):
            before = cell.get('previously_published_' + field)
            if before is not None:
                superseded.append({'cell': key, 'field': field, 'now': cell[field], 'before': before,
                                   'delta': round(cell[field] - before, 3)})
    ...
    return {'passed': len(data) == 120 and all(x['matched'] for x in checks) and (result_csv is not None or generated == archived),
            ...
            'scope': 'FCP/BSP InS/OOS from the archived sweep and CPBSD-A from the 300-second rerun that follows the protocol the paper states; 36 statistics. Out-of-sample profit depends on which in-sample optimum the solver returns, so it is reproducible only under the same solver configuration.',
            'cpbsd_a_superseded_values': superseded,
            'details': checks}
